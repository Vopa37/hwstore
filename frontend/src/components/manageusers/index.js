       ...
       <div className="w-80 m-auto">
           ...
           <AnimatePresence>
            ...
            { users && users.map((user,index)=>(
                <div className="text-black w-100 bg-white my-4 d-block" key={index}>
                    <div className="w-100">
                    ...
                    <div className="w-100 d-flex flex-column justify-content-center">
                        <Button className="w-50 mx-auto" onClick={()=>{setEditUser(user)}}>Upravit</Button>
                        {!user.admin && <Button className="w-50 mx-auto" onClick={()=>{deleteUser(user._id,setUsers)}}>Odstranit</Button> }
                    </div>

                    <AnimatePresence>
                        {editUser &&
                        <Modal toggle={setEditUser}>
                            <EditUser admin={user.admin} user={editUser} toggle={setEditUser}/>
                        </Modal>
                        }
                    </AnimatePresence>
                </div>
